...
var taskbarAppIcon = Utils.defineClass({
    ...
    _init(appInfo, panel, iconParams, previewMenu) {
        this.dtpPanel = panel;
        this._nWindows = 0;
        ...
        if (!this._checkForIcon(appInfo.app)) {
            this.icon._createIconTexture = function (size) {
                ...
                this.iconSize = size;
                this.icon = new St.Icon({ icon_name: 'file-unknown',
                    icon_size: size });

                let path = `${Me.path}/img/file_unknown.svg`;
                this.icon.set_gicon(Gio.Icon.new_for_string(path));
...
var taskbarSecondaryMenu = Utils.defineClass({
    ...
    _closeWindowInstance(metaWindow) {
...
function adjustMenuRedisplay(menuProto) {
    menuProto[menuRedisplayFunc] = function () {
        this._dtpRedisplay(menuRedisplayFunc);
    };
}
