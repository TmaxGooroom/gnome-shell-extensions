...
const Settings = new Lang.Class({ // eslint-disable-line no-restricted-properties
    ...
    _displayPanelPositionsForMonitor(monitorIndex) {
        ...
        labels[Pos.NOTI_CENTER] = _('Notification Center');
    ...
    _showFileChooser(title, params, acceptBtn, acceptHandler) {
        let dialog = new Gtk.FileChooserDialog(mergeObjects({ title, transient_for: this.widget.get_toplevel() }, params));
    ...
    _SignalHandler: {
        ...
        position_bottom_button_clicked_cb(button) {
            if (!this._ignorePositionRadios && button.get_active())
                this._setPanelPosition(Pos.BOTTOM);
        },

        position_top_button_clicked_cb(button) {
            if (!this._ignorePositionRadios && button.get_active())
                this._setPanelPosition(Pos.TOP);
        },

        position_left_button_clicked_cb(button) {
            if (!this._ignorePositionRadios && button.get_active())
                this._setPanelPosition(Pos.LEFT);
        },

        position_right_button_clicked_cb(button) {
            if (!this._ignorePositionRadios && button.get_active())
                this._setPanelPosition(Pos.RIGHT);
        },
