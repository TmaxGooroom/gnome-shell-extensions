...
var dtpPanel = Utils.defineClass({ // eslint-disable-line no-unused-vars
    ...
    _init(panelManager, monitor, panelBox, isStandalone) {
      ...
      monitor === panelManager.dtpPrimaryMonitor;
        ...
        this._elementGroups = [];
        this.cornerSize = 0;
        this._emptyActor = new St.BoxLayout({ name: 'dummy' });
    ...
    disable() {
        ...
        if (!this.isStandalone) {
            ...
            this._rightBox.disconnect(this.moveNotificationCenterId);
            Main.extensionManager.disconnect(this.revertNotificationCenterId);
            ...
            if (!Main.sessionMode.isLocked) {
                [['activities', 0], ['aggregateMenu', -1], ['dateMenu', 0], ['NotificationCenter', -1]].forEach(b => {
    ...
    _setAllocationMap() {
        ...
        if (!this.isStandalone && this.statusArea.NotificationCenter)
            ...
            setMap(Pos.NOTI_CENTER, this._emptyActor);
    ...
    // move notificationcenter from rightbox to panel actor when NotiCenter loaded later than dtpPanel
        this.moveNotificationCenterId = this._rightBox.connect('actor-added', (addedBox, addedActor) => {
            ...
            if (parent)
                parent.remove_child(container);
            this.panel.actor.add_child(container);
            this.panel.actor.remove_child(this._emptyActor);

            this.allocationMap[Pos.NOTI_CENTER].actor = this.statusArea.NotificationCenter.container;
        });

        const state = imports.misc.extensionUtils.ExtensionState;
        ...
        this.revertNotificationCenterId = Main.extensionManager.connect('extension-state-changed', (data, extension) => {
            ...
            this.allocationMap[Pos.NOTI_CENTER].actor = this._emptyActor;
        });
    },
